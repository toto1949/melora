...
insert into packages(id,slug,name,description,price_cents) values('22222222-2222-4222-8222-222222222222','essential-song','Personalized AI Song','Test',999);
...
begin
 ...
 assert (select total_cents=999 and tax_cents=0 from orders where id=oid),'Hardcoded tax';
 begin
 ...
 begin
  perform apply_stripe_event('evt_bad','checkout.session.completed',oid,'paid','cs_test','pi_test','price_other',999,0,0,'Customer');
 ...
 changed:=apply_stripe_event('evt_paid','checkout.session.completed',oid,'paid','cs_test','pi_test','price_test',1159,160,0,'Customer');
 assert changed,'Valid event not applied';
 assert (select payment_status='paid' and total_cents=1159 from orders where id=oid),'Order not paid';
 ...
 changed:=apply_stripe_event('evt_paid','checkout.session.completed',oid,'paid','cs_test','pi_test','price_test',1159,160,0,'Customer'); assert not changed,'Duplicate event processed';
 perform apply_stripe_event('evt_other','checkout.session.async_payment_succeeded',oid,'paid','cs_test','pi_test','price_test',1159,160,0,'Customer');
 ...
 perform apply_stripe_event('evt_refund','charge.refunded',oid,'refunded','cs_test','pi_test','price_test',1159,160,1159,null);
 ...
 perform apply_stripe_event('evt_late','checkout.session.completed',oid,'paid','cs_test','pi_test','price_test',1159,160,0,null);
