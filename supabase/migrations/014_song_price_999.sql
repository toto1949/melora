-- New orders cost USD 9.99; historical order amounts remain unchanged.
begin;
update public.packages set price_cents=999,currency='usd' where slug='essential-song';
...
begin
  ...
  insert into orders(
    ...
    'MTM-'||upper(encode(gen_random_bytes(8),'hex')),p_user,p_project,p_package,999,999,'usd',p_email,p_phone,
...
begin
  ...
  if p_state='paid' then
    if o.stripe_checkout_session_id is distinct from p_session or o.stripe_price_id is distinct from p_price
      or o.currency<>'usd' or o.subtotal_cents not in (999,1999) or o.discount_cents<>0 or p_total<o.subtotal_cents or p_tax<0 or p_total<>o.subtotal_cents+p_tax then
